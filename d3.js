d3 = function() {
  ...
  d3.svg.axis = function() {
    var scale = d3.scale.linear(), orient = d3_svg_axisDefaultOrient, tickMajorSize = 6, tickMinorSize = 6, tickEndSize = 6, tickPadding = 3, tickArguments = [ 10 ], tickValues = null, tickFormat_, minorTickArguments = null;
    function axis(g) {
      g.each(function() {
        var g = d3.select(this);
        var ticks = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain() : tickValues, tickFormat = tickFormat_ == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : String : tickFormat_;
        if (minorTickArguments) {
          var major = d3.set(ticks), minorTicks = scale.ticks.apply(scale, minorTickArguments).filter(function(d) {
            return !major.has(d);
          }), minorTick = g.selectAll(".tick.minor").data(minorTicks, String), minorTickEnter = minorTick.enter().insert("line", ".tick").attr("class", "tick minor").style("opacity", 1e-6), minorTickExit = d3.transition(minorTick.exit()).style("opacity", 1e-6).remove(), minorTickUpdate = d3.transition(minorTick).style("opacity", 1);
        } else {
