!function() {
  ...
  function d3_geo_projectionRadiansRotate(δλ, δφ, δγ, stream) {
    return d3_geo_transformPoint(stream, δλ && (δφ || δγ) ? d3_geo_projectionRadiansRotateλφγ(δλ, δφ, δγ, stream) : δλ ? d3_geo_projectionRadiansRotateλ(δλ, stream) : δφ || δγ ? d3_geo_projectionRadiansRotateφγ(δλ, δφ, δγ, stream) : function(x, y) {
  ...
  function d3_geo_projectionRadiansRotateφγ(δφ, δγ, stream) {
    ...
    return function(λ, φ) {
      ...
      stream.point(Math.atan2(y * cosδγ - k * sinδγ, x * cosδφ - z * sinδφ), d3_asin(k * cosδγ + y * sinδγ));
    };
  }
  function d3_geo_projectionRadiansRotateλφγ(δλ, δφ, δγ, stream) {
    var cosδφ = Math.cos(δφ), sinδφ = Math.sin(δφ), cosδγ = Math.cos(δγ), sinδγ = Math.sin(δγ);
    return function(λ, φ) {
      var cosφ = Math.cos(φ), x = Math.cos(λ += δλ) * cosφ, y = Math.sin(λ) * cosφ, z = Math.sin(φ), k = z * cosδφ + x * sinδφ;
      stream.point(Math.atan2(y * cosδγ - k * sinδγ, x * cosδφ - z * sinδφ), d3_asin(k * cosδγ + y * sinδγ));
    };
  }
