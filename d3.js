  ...
  d3.behavior.drag = function() {
    ...
    function mousedown() {
      ...
      function dragmove() {
        ...
        var p = point(), x = p[0] + offset[0], y = p[1] + offset[1], dx = p[0] - origin_[0], dy = p[1] - origin_[1];
        moved |= dx | dy;
        origin_ = p;
        if (inertia) {
          addSamples(x, y);
      ...
      function step() {
        ...
        var now = Date.now(), dt = now - t, ax = damping * vx, ay = damping * vy, dx = vx * dt + .5 * ax * dt * dt, dy = vy * dt + .5 * ay * dt * dt;
        x += dx;
        y += dy;
        vx += ax * dt;
        vy += ay * dt;
