(function() {
  ...
  d3_QL._genFieldsNameGen = function(fields) {
  ...
  d3_QL._insertGroupFields = function(retObj, fields, keyName) {
    var keys = keyName.split(d3_QL._keyDelimiter);
    for (var i = 0, l = keys.length; i < l; i++) {
      retObj[fields[i]] = keys[i];
    }
  };
  d3_QL.prototype.select = function(fields) {
    var keys = d3.keys(fields), keylen = keys.length;
    var resArray = [];
  ...
  d3_QL.prototype.where = function(cond) {
    var resArray = [];
    for (var i = 0, l = this.length; i < l; i++) {
  ...
  d3_QL.prototype.join = function(array, onCond) {};
  d3_QL.prototype.order = function(orders) {
    var _orders = orders;
  ...
  d3_QL.prototype.group = function(fields, ops, init) {
    var len = this.length;
    var _retHash = {};
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (key) {
        if (!_retHash.hasOwnProperty(key)) _retHash[key] = {};
        for (var k in ops) {
          if (!ops.hasOwnProperty(k)) break;
          if (!_retHash[key].hasOwnProperty(k)) _retHash[key][k] = init[k] || 0;
  ...
  d3_QL.prototype.distinct = function(field, sorted) {
    var keys = {};
    sorted = sorted || false;
    for (var i = 0, l = this.length; i < l; i++) {
      if (this[i].hasOwnProperty(field)) keys[this[i][field]] = true;
    }
    ...
    for (var key in cond) {
      if (cond.hasOwnProperty(key)) {
        ...
        if (typeof cond[key] === "function") {
          if (!cond[key](obj[key])) return false;
        } else if (cond[key] instanceof RegExp) {
          if (!obj[key].match(cond[key])) return false;
        } else if (typeof cond[key] === "object") {} else {
          if (obj[key] !== cond[key]) {
            return false;
          }
        }
