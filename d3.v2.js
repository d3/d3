    ...
    function resample(context) {
      ...
      function resampleLineTo(x0, y0, λ0, φ0, x1, y1, λ1, φ1, depth) {
        ...
        if (distance2 > 4 * δ2 && depth--) {
          var sinφ0 = Math.sin(φ0), cosφ0 = Math.cos(φ0), sinφ1 = Math.sin(φ1), cosφ1 = Math.cos(φ1), λ2 = (λ0 + λ1) / 2, φ2 = Math.asin(Math.max(-1, Math.min(1, (sinφ0 + sinφ1) / (Math.SQRT2 * Math.sqrt(1 + sinφ0 * sinφ1 + cosφ0 * cosφ1 * Math.cos(λ1 - λ0)))))), p = projectPoint(λ2, φ2), x2 = p[0], y2 = p[1], dz = dx * (y0 - y2) - dy * (x0 - x2);
