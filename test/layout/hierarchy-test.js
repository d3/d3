...
suite.addBatch({
  "hierarchy": {
    topic: function() {
      return d3.layout.treemap().value(function(d) { return d.size; }); // hierarchy is abstract, so test a subclass
    },
    "doesn't overwrite the value of a node that has an empty children array": function(hierarchy) {
      var nodes = hierarchy.sticky(true).nodes({size: 1, children: []});
    ...
    "negative values are treated as zeroes": function(hierarchy) {
      var nodes = hierarchy.sticky(true).nodes({size: -1});
      assert.equal(nodes[0].value, 0);
      hierarchy.nodes(nodes[0]);
      assert.equal(nodes[0].value, 0);
    }
