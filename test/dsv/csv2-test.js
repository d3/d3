...
var suite = vows.describe("d3.csv2");

suite.addBatch({
  "csv2": {
    topic: load("dsv/csv2").expression("d3.csv2").document(),

    "on a sample file": {
      topic: function(csv2) {
        csv2("test/data/sample2.csv", this.callback);
      },
      "invokes the callback with the parsed CSV": function(csv2) {
        assert.deepEqual(csv2, [{"Hello":"42","World":"\"fish\""}]);
      },
      "overrides the mime type to text/csv": function(csv2) {
    ...
    "when specifying a row conversion function": {
      topic: function(csv2) {
        csv2("test/data/sample2.csv", function(row) {
          row.Hello = -row.Hello;
          return row;
        }, this.callback);
      },
      "invokes the callback with the parsed CSV": function(csv2) {
        assert.strictEqual(csv2[0].Hello, -42);
      }
    },
    ...
    "when loading a file that does not exist": {
      topic: function(csv2) {
        ...
        csv2("//does/not/exist.csv", function(error, csv2) {
          callback(null, csv2);
        });
      },
      "invokes the callback with undefined": function(csv2) {
        assert.isUndefined(csv2);
      }
    },
    ...
    "parse with row function": {
      "invokes the row function for every row in order": function(csv2) {
        var rows = [];
      ...
      "returns an array of the row function return values": function(csv2) {
        assert.deepEqual(csv2.parse("a;b;c\n1;2;3\n", function(row) { return row; }), [{a: "1", b: "2", c: "3"}]);
      },
      "skips rows if the row function returns null or undefined": function(csv2) {
