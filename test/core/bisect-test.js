  ...
  "bisectLeft": {
    ...
    "finds the index of an exact match": function(bisect) {
      ...
      var array = [{k:1}, {k:2}, {k:3}],
          keyfn = function (d) { return d.k; };
      assert.equal(bisect(array, keyfn, 1), 0);
      assert.equal(bisect(array, keyfn, 2), 1);
      assert.equal(bisect(array, keyfn, 3), 2);
    },
    "finds the index of the first match": function(bisect) {
      ...
      var array = [{k:1}, {k:2}, {k:2}, {k:3}],
          keyfn = function (d) { return d.k; };
      assert.equal(bisect(array, keyfn, 1), 0);
      assert.equal(bisect(array, keyfn, 2), 1);
      assert.equal(bisect(array, keyfn, 3), 3);
    },
    "finds the insertion point of a non-exact match": function(bisect) {
      ...
      var array = [{k:1}, {k:2}, {k:3}],
          keyfn = function (d) { return d.k; };
      assert.equal(bisect(array, keyfn, 0.5), 0);
      assert.equal(bisect(array, keyfn, 1.5), 1);
      assert.equal(bisect(array, keyfn, 2.5), 2);
      assert.equal(bisect(array, keyfn, 3.5), 3);
    },
    "observes the optional lower bound": function(bisect) {
      ...
      var array = [{k:1}, {k:2}, {k:3}, {k:4}, {k:5}],
          keyfn = function (d) { return d.k; };
      assert.equal(bisect(array, keyfn, 0, 2), 2);
      assert.equal(bisect(array, keyfn, 1, 2), 2);
      assert.equal(bisect(array, keyfn, 2, 2), 2);
      assert.equal(bisect(array, keyfn, 3, 2), 2);
      assert.equal(bisect(array, keyfn, 4, 2), 3);
      assert.equal(bisect(array, keyfn, 5, 2), 4);
      assert.equal(bisect(array, keyfn, 6, 2), 5);
    },
    "observes the optional bounds": function(bisect) {
      ...
      var array = [{k:1}, {k:2}, {k:3}, {k:4}, {k:5}],
          keyfn = function (d) { return d.k; };
      assert.equal(bisect(array, keyfn, 0, 2, 3), 2);
      assert.equal(bisect(array, keyfn, 1, 2, 3), 2);
      assert.equal(bisect(array, keyfn, 2, 2, 3), 2);
      assert.equal(bisect(array, keyfn, 3, 2, 3), 2);
      assert.equal(bisect(array, keyfn, 4, 2, 3), 3);
      assert.equal(bisect(array, keyfn, 5, 2, 3), 3);
      assert.equal(bisect(array, keyfn, 6, 2, 3), 3);
    }
  }
});
...
suite.addBatch({
  "bisectRight": {
    ...
    "finds the index after an exact match": function(bisect) {
      ...
      var array = [{k:1}, {k:2}, {k:3}],
          keyfn = function (d) { return d.k; };
      assert.equal(bisect(array, keyfn, 1), 1);
      assert.equal(bisect(array, keyfn, 2), 2);
      assert.equal(bisect(array, keyfn, 3), 3);
    },
    "finds the index after the last match": function(bisect) {
      ...
      var array = [{k:1}, {k:2}, {k:2}, {k:3}],
          keyfn = function (d) { return d.k; };
      assert.equal(bisect(array, keyfn, 1), 1);
      assert.equal(bisect(array, keyfn, 2), 3);
      assert.equal(bisect(array, keyfn, 3), 4);
    },
    "finds the insertion point of a non-exact match": function(bisect) {
      var array = [1, 2, 3];
      assert.equal(bisect(array, 0.5), 0);
    ...
    "observes the optional lower bound": function(bisect) {
      ...
      assert.equal(bisect(array, keyfn, 1, 2), 2);
      assert.equal(bisect(array, keyfn, 2, 2), 2);
      assert.equal(bisect(array, keyfn, 3, 2), 3);
      assert.equal(bisect(array, keyfn, 4, 2), 4);
      assert.equal(bisect(array, keyfn, 5, 2), 5);
      assert.equal(bisect(array, keyfn, 6, 2), 5);
    },
    "observes the optional bounds": function(bisect) {
      var array = [1, 2, 3, 4, 5];
      ...
      assert.equal(bisect(array, keyfn, 1, 2, 3), 2);
      assert.equal(bisect(array, keyfn, 2, 2, 3), 2);
      assert.equal(bisect(array, keyfn, 3, 2, 3), 3);
      assert.equal(bisect(array, keyfn, 4, 2, 3), 3);
      assert.equal(bisect(array, keyfn, 5, 2, 3), 3);
      assert.equal(bisect(array, keyfn, 6, 2, 3), 3);
    }
