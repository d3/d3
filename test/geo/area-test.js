  ...
  "area": {
    ...
    "Polygon": {
      ...
      "semilune": function(area) {
        assert.inDelta(area({type: "Polygon", coordinates: [[[0, 0], [0, 90], [90, 0], [0, 0]]]}), pi / 2, 1e-6);
      },
      "lune": function(area) {
        assert.inDelta(area({type: "Polygon", coordinates: [[[0, 0], [0, 90], [90, 0], [0, -90], [0, 0]]]}), pi, 1e-6);
      },
      "hemispheres": {
        "North": function(area) {
          assert.inDelta(area({type: "Polygon", coordinates: [[[0, 0], [-90, 0], [180, 0], [90, 0], [0, 0]]]}), 2 * pi, 1e-6);
        },
        "South": function(area) {
          assert.inDelta(area({type: "Polygon", coordinates: [[[0, 0], [90, 0], [180, 0], [-90, 0], [0, 0]]]}), 2 * pi, 1e-6);
        },
        "East": function(area) {
          assert.inDelta(area({type: "Polygon", coordinates: [[[0, 0], [0, 90], [180, 0], [0, -90], [0, 0]]]}), 2 * pi, 1e-6);
        },
        "West": function(area) {
          assert.inDelta(area({type: "Polygon", coordinates: [[[0, 0], [0, -90], [180, 0], [0, 90], [0, 0]]]}), 2 * pi, 1e-6);
        }
      ...
      "graticule outline": {
        "sphere": function(area) {
          assert.inDelta(area(_.geo.graticule().extent([[-180, -90], [180, 90]]).outline()), 4 * pi, 1e-5);
        },
        "hemisphere": function(area) {
          assert.inDelta(area(_.geo.graticule().extent([[-180, 0], [180, 90]]).outline()), 2 * pi, 1e-5);
        },
        "semilune": function(area) {
          assert.inDelta(area(_.geo.graticule().extent([[0, 0], [90, 90]]).outline()), pi / 2, 1e-5);
        }
      },
      "circles": {
        "hemisphere": function(area) {
          assert.inDelta(area(_.geo.circle().angle(90)()), 2 * pi, 1e-5);
        },
        "60°": function(area) {
          assert.inDelta(area(_.geo.circle().angle(60).precision(.1)()), pi, 1e-5);
        },
        "60° North": function(area) {
          assert.inDelta(area(_.geo.circle().angle(60).precision(.1).origin([0, 90])()), pi, 1e-5);
        },
        "45°": function(area) {
          assert.inDelta(area(_.geo.circle().angle(45).precision(.1)()), pi * (2 - Math.SQRT2), 1e-5);
        },
        "45° North": function(area) {
          assert.inDelta(area(_.geo.circle().angle(45).precision(.1).origin([0, 90])()), pi * (2 - Math.SQRT2), 1e-5);
        },
        "45° South": function(area) {
          assert.inDelta(area(_.geo.circle().angle(45).precision(.1).origin([0, -90])()), pi * (2 - Math.SQRT2), 1e-5);
        },
        "135°": function(area) {
          assert.inDelta(area(_.geo.circle().angle(135).precision(.1)()), pi * (2 + Math.SQRT2), 1e-5);
        },
        "135° North": function(area) {
          assert.inDelta(area(_.geo.circle().angle(135).precision(.1).origin([0, 90])()), pi * (2 + Math.SQRT2), 1e-5);
        },
        ...
        "huge": function(area) {
          assert.inDelta(area(_.geo.circle().angle(180 - 1e-6).precision(.1)()), 4 * pi, 1e-6);
        },
      ...
      "stripes": {
        "45°, -45°": function(area) {
          assert.inDelta(area(stripes(45, -45)), pi * 2 * Math.SQRT2, 1e-5);
        },
        "-45°, 45°": function(area) {
          assert.inDelta(area(stripes(-45, 45)), pi * 2 * (2 - Math.SQRT2), 1e-5);
        },
        "45°, 30°": function(area) {
          assert.inDelta(area(stripes(45, 30)), pi * (Math.SQRT2 - 1), 1e-5);
        }
      }
    },
    "MultiPolygon": {
    ...
    "Sphere": function(area) {
      assert.equal(area({type: "Sphere"}), 4 * pi);
    },
    "GeometryCollection": function(area) {
      assert.equal(area({type: "GeometryCollection", geometries: [{type: "Sphere"}]}), 4 * pi);
    },
    "FeatureCollection": function(area) {
      assert.equal(area({type: "FeatureCollection", features: [{type: "Feature", geometry: {type: "Sphere"}}]}), 4 * pi);
    },
    "Feature": function(area) {
      assert.equal(area({type: "Feature", geometry: {type: "Sphere"}}), 4 * pi);
    }
  }
});
