...
suite.addBatch({
  "circle": {
    topic: load("geo/circle", "arrays/range"),
    "generates a Polygon": function(d3) {
      var o = d3.geo.circle()();
    ...
    "origin([0, 90])": function(d3) {
      var o = d3.geo.circle().origin([0, 90])();
      assert.equal(o.type, "Polygon");
      assert.inDelta(o.coordinates, [d3.range(360, -1, -6).map(function(x) { return [x >= 180 ? x - 360 : x, 0]; })], 1e-6);
    },
    "origin([45, 45])": function(d3) {
      var o = d3.geo.circle().origin([45, 45]).angle(0)();
      assert.equal(o.type, "Polygon");
