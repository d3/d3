...
suite.addBatch({
  "quantile": {
    topic: load("scale/quantile"),
    "has the empty domain by default": function(d3) {
      assert.isEmpty(d3.scale.quantile().domain());
    },
    "has the empty range by default": function(d3) {
      assert.isEmpty(d3.scale.quantile().range());
    },
    "uses the R-7 algorithm to compute quantiles": function(d3) {
