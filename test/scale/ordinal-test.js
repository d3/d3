...
suite.addBatch({
  "ordinal": {
    ...
    "rangeRoundBands": {
      ...
      "can be set to a descending range": function(d3) {
        var x = d3.scale.ordinal().domain(["a", "b", "c"]).rangeRoundBands([100, 0]);
        assert.deepEqual(x.range(), [67, 34, 1]);
        assert.equal(x.rangeBand(), 33);
        var x = d3.scale.ordinal().domain(["a", "b", "c"]).rangeRoundBands([100, 0], .2);
    ...
    "rangeExtent": {
      "returns the continuous range": function(d3) {
        var x = d3.scale.ordinal().domain(["a", "b", "c"]).rangePoints([20, 120]);
        assert.deepEqual(x.rangeExtent(), [20, 120]);
        var x = d3.scale.ordinal().domain(["a", "b", "c"]).rangeBands([10, 110]);
        assert.deepEqual(x.rangeExtent(), [10, 110]);
        var x = d3.scale.ordinal().domain(["a", "b", "c"]).rangeRoundBands([0, 100]);
        assert.deepEqual(x.rangeExtent(), [0, 100]);
        var x = d3.scale.ordinal().domain(["a", "b", "c"]).range([0, 20, 100]);
        assert.deepEqual(x.rangeExtent(), [0, 100]);
      },
      "can handle descending ranges": function(d3) {
        var x = d3.scale.ordinal().domain(["a", "b", "c"]).rangeBands([100, 0]);
        assert.deepEqual(x.rangeExtent(), [0, 100]);
      }
    },
    ...
    "copy": {
      "changes to the domain are isolated": function(d3) {
        var x = d3.scale.ordinal().range(["foo", "bar"]), y = x.copy();
        x.domain([1, 2]);
      ...
      "changes to the range type are isolated": function(d3) {
        var x = d3.scale.ordinal().domain([0, 1]).rangeBands([0, 1], .2), y = x.copy();
        x.rangePoints([1, 2]);
