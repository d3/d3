...
function F(a){var g,e,f,d,c;if(d=/([a-z]+)\((.*)\)/i.exec(a)){c=d[2].split(",");switch(d[1]){case "hsl":return O(parseFloat(c[0]),parseFloat(c[1])/100,parseFloat(c[2])/100);case "rgb":return{r:G(c[0]),g:G(c[1]),b:G(c[2])}}}if(d=z[a])return d;if(a==null)return z.black;if(a.charAt(0)=="#"){if(a.length==4){g=a.charAt(1);g+=g;e=a.charAt(2);e+=e;f=a.charAt(3);f+=f}else if(a.length==7){g=a.substring(1,3);e=a.substring(3,5);f=a.substring(5,7)}g=parseInt(g,16);e=parseInt(e,16);f=parseInt(f,16)}return{r:g,
g:e,b:f}}function O(a,g,e){function f(b){if(b>360)b-=360;else if(b<0)b+=360;if(b<60)return d+(c-d)*b/60;if(b<180)return c;if(b<240)return d+(c-d)*(240-b)/60;return d}var d,c;a%=360;if(a<0)a+=360;g=g<0?0:g>1?1:g;e=e<0?0:e>1?1:e;c=e<=0.5?e*(1+g):e+g-e*g;d=2*e-c;return{r:Math.round(f(a+120)*255),g:Math.round(f(a)*255),b:Math.round(f(a-120)*255)}}function G(a){var g=parseFloat(a);return a.charAt(a.length-1)=="%"?Math.round(g*2.55):g}function P(){function a(f,d){var c=Object.create(g);c.pop=f;c.data=function(b){var h,
...
e;k[0]=b.data;b[d]=f.bind.apply(b,k);delete b.value;delete b.name}this.value=function(){return this[d].apply(this,arguments)}}}function Q(a,g){var e=this.value,f=a.length,d,c=this.key,b,h,i,j,n=[],o=[],v=[],p,s,m;if(typeof e=="function"){b=k.shift();e=e.apply(null,k);k.unshift(b)}d=e.length;if(c){b=c.name;h=c.value;p={};s={};m={};if(b.local)for(c=0;c<f;++c){if(j=a[c].node){i=j.getAttributeNS(b.space,b.local);if(i!=null)p[i]=j}}else for(c=0;c<f;++c)if(j=a[c].node){i=j.getAttribute(b);if(i!=null)p[i]=
...
function $(a,g){var e=a.length,f=this.bound||(this.bound=this.value),d,c,b;if(f&&f.bind){d=[];for(c=0;c<e;++c){k[0]=(b=a[c]).data;b.value=b.data;b.data_=f.bind.apply(b,k);delete b.value}this.value=function(){k.unshift(null);for(c=0;c<e;++c){k[0]=(b=a[c]).data;d[c]=b.data_.apply(b,k)}k.shift();return d}}else Q.call(this,a,g)}function fa(a){var g=a.length,e=this.selector,f,d,c,b,h;if(e==null)for(d=0;d<g;++d){h=a[d].node;h.parentNode.removeChild(h)}else for(d=0;d<g;++d){f=a[d].node.querySelectorAll(e);
c=0;for(b=f.length;c<b;c++){h=f[c];h.parentNode.removeChild(h)}}}function ha(a){function g(j){return function(n){var o=k;try{k=i;l.event=n;for(b=0;b<f;++b)e[b].impl(j,u)}finally{delete l.event;k=o}}}var e=this.actions,f=e.length,d=a.length,c="on"+this.type,b=0,h,i=k.slice();if(f)for(;b<d;++b){h=a[b];h.node[c]=g([h])}else for(;b<d;++b)a[b].node[c]=null}function R(a,g){var e=[],f=a.length,d=this.filter,c,b;for(c=0;c<f;++c){k[0]=(b=a[c]).data;d.apply(b,k)&&e.push(b)}g(this.actions,e)}function H(a,g){var e=
...
d.appendChild(document.createTextNode(c))}else for(f=0;f<g;++f){for(d=a[f].node;d.lastChild;)d.removeChild(d.lastChild);d.appendChild(document.createTextNode(e))}}function ka(a){function g(w){var q=k;for(m=0;m<s;++m)(r=a[m]).node.interval=w;try{l.time=0;k=B;S(d,a)}finally{delete l.time;k=q}}function e(){var w=k,q=a.filter(function(la){return la.node.interval==o}),C=q.length,J=Date.now(),x,T=true;try{k=B;for(m=0;m<C;++m){r=q[m];x=(J-b-r.delay)/j;if(!(x<0)){if(x>1)x=1;else T=false;l.time=n(x);for(t=
0;t<v;++t)d[t].impl([r],u);if(x==1){for(t=0;t<p;++t)c[t].impl([r],u);r.delay=Infinity}}}}finally{delete l.time;k=w}T&&clearInterval(o)}function f(){var w=k,q=(Date.now()-b-h)/j,C=a.filter(function(J){return J.node.interval==o});try{k=B;l.time=n(q<0?0:q>1?1:q);for(m=0;m<v;++m)d[m].impl(C,u)}finally{delete l.time;k=w}if(q>=1){clearInterval(o);try{k=B;for(m=0;m<p;++m)c[m].impl(C,u)}finally{k=w}}}var d=this.actions,c=this.endActions,b=Date.now(),h=this.delay,i=Infinity,j=this.duration,n=this.ease,o,v=
d.length,p=c.length,s=a.length,m,t,r,K,B=k.slice();if(typeof h=="function"){for(m=0;m<s;++m){k[0]=(r=a[m]).data;K=r.delay=h.apply(r,k);if(K<i)i=K}setTimeout(function(){g(o=setInterval(e,24))},i)}else setTimeout(function(){g(o=setInterval(f,24))},h)}function S(a,g){var e=a.length,f,d;for(d=0;d<e;++d){f=a[d];f.bind&&f.bind(g,S)}}var l=D.d3={};l.version="0.0.0";var A={prefix:{svg:"http://www.w3.org/2000/svg",xhtml:"http://www.w3.org/1999/xhtml",xlink:"http://www.w3.org/1999/xlink",xml:"http://www.w3.org/XML/1998/namespace",
...
for(var U in z)z[U]=F(z[U]);l.hsl=function(a,g,e){a=O(a,g,e);return"rgb("+a.r+","+a.g+","+a.b+")"};var k=[];l.select=function(a){return P().select(a)};l.selectAll=function(a){return P().selectAll(a)}})(this);
