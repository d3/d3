(function(){var n=null;d3={version:"0.28.1"};if(!Date.now)Date.now=function(){return+new Date};if(!Object.create)Object.create=function(a){function b(){}b.prototype=a;return new b};function x(a){return Array.prototype.slice.call(a)}function y(a){return typeof a=="function"?a:function(){return a}}d3.ascending=function(a,b){return a<b?-1:a>b?1:0};d3.descending=function(a,b){return b<a?-1:b>a?1:0};d3.merge=function(a){return Array.prototype.concat.apply([],a)};
d3.split=function(a,b){var e=[],f=[],c,d=-1,h=a.length;if(arguments.length<2)b=aa;for(;++d<h;)if(b.call(f,c=a[d],d)){e.push(f);f=[]}else f.push(c);e.push(f);return e};function aa(a){return a==n}function E(a){return a.replace(/(^\s+)|(\s+$)/g,"").replace(/\s+/g," ")}function ba(a,b){b=x(arguments);b[0]=this;a.apply(this,b);return this}
d3.range=function(a,b,e){if(arguments.length==1){b=a;a=0}if(e==n)e=1;if((b-a)/e==Infinity)throw Error("infinite range");var f=[],c=-1,d;if(e<0)for(;(d=a+e*++c)>b;)f.push(d);else for(;(d=a+e*++c)<b;)f.push(d);return f};d3.requote=function(a){return a.replace(ca,"\\$&")};var ca=/[\\\^\$\*\+\?\[\]\(\)\.\{\}]/g;
d3.xhr=function(a,b,e){var f=new XMLHttpRequest;if(arguments.length<3)e=b;else b&&f.overrideMimeType(b);f.open("GET",a,true);f.onreadystatechange=function(){if(f.readyState==4)e(f.status<300?f:n)};f.send(n)};d3.text=function(a,b,e){if(arguments.length<3){e=b;b=n}d3.xhr(a,b,function(f){e(f&&f.responseText)})};d3.json=function(a,b){d3.text(a,"application/json",function(e){b(e?JSON.parse(e):n)})};
d3.html=function(a,b){d3.text(a,"text/html",function(e){if(e!=n){var f=document.createRange();f.selectNode(document.body);e=f.createContextualFragment(e)}b(e)})};d3.xml=function(a,b,e){if(arguments.length<3){e=b;b=n}d3.xhr(a,b,function(f){e(f&&f.responseXML)})};
...
d3.interpolateString=function(a,b){var e,f,c=0,d=[],h=[],g,i;for(f=0;e=H.exec(b);++f){e.index&&d.push(b.substring(c,e.index));h.push({a:d.length,x:e[0]});d.push(n);c=H.lastIndex}c<b.length&&d.push(b.substring(c));f=0;for(g=h.length;(e=H.exec(a))&&f<g;++f){i=h[f];if(i.x==e[0]){if(i.a)if(d[i.a+1]==n){d[i.a-1]+=i.x;d.splice(i.a,1);for(e=f+1;e<g;++e)h[e].a--}else{d[i.a-1]+=i.x+d[i.a+1];d.splice(i.a,2);for(e=f+1;e<g;++e)h[e].a-=2}else if(d[i.a+1]==n)d[i.a]=i.x;else{d[i.a]=i.x+d[i.a+1];d.splice(i.a+1,1);
for(e=f+1;e<g;++e)h[e].a--}h.splice(f,1);g--;f--}else i.x=d3.interpolateNumber(parseFloat(e[0]),parseFloat(i.x))}for(;f<g;){i=h.pop();if(d[i.a+1]==n)d[i.a]=i.x;else{d[i.a]=i.x+d[i.a+1];d.splice(i.a+1,1)}g--}if(d.length==1)return d[0]==n?h[0].x:function(){return b};return function(j){for(f=0;f<g;++f)d[(i=h[f]).a]=i.x(j);return d.join("")}};
...
function I(a,b,e){var f,c,d,h,g;if(h=/([a-z]+)\((.*)\)/i.exec(a)){g=h[2].split(",");switch(h[1]){case "hsl":return e(parseFloat(g[0]),parseFloat(g[1])/100,parseFloat(g[2])/100);case "rgb":return b(L(g[0]),L(g[1]),L(g[2]))}}if(e=G[a])return b(e.r,e.g,e.b);if(a==n)return b(0,0,0);if(a.charAt(0)=="#"){if(a.length==4){f=a.charAt(1);f+=f;c=a.charAt(2);c+=c;d=a.charAt(3);d+=d}else if(a.length==7){f=a.substring(1,3);c=a.substring(3,5);d=a.substring(5,7)}f=parseInt(f,16);c=parseInt(c,16);d=parseInt(d,16)}return b(f,
...
d3.svg.line=function(){function a(d){return d.length<1?n:"M"+c(W(this,d,b,e))}var b=Na,e=Oa,f="linear",c=X[f];a.x=function(d){if(!arguments.length)return b;b=d;return a};a.y=function(d){if(!arguments.length)return e;e=d;return a};a.interpolate=function(d){if(!arguments.length)return f;c=X[f=d];return a};return a};
