(function(){var o=null;d3={version:"0.28.3"};if(!Date.now)Date.now=function(){return+new Date};if(!Object.create)Object.create=function(a){function b(){}b.prototype=a;return new b};function x(a){return Array.prototype.slice.call(a)}function y(a){return typeof a=="function"?a:function(){return a}}d3.ascending=function(a,b){return a<b?-1:a>b?1:0};d3.descending=function(a,b){return b<a?-1:b>a?1:0};d3.merge=function(a){return Array.prototype.concat.apply([],a)};
d3.split=function(a,b){var c=[],f=[],d,e=-1,i=a.length;if(arguments.length<2)b=aa;for(;++e<i;)if(b.call(f,d=a[e],e)){c.push(f);f=[]}else f.push(d);c.push(f);return c};function aa(a){return a==o}function E(a){return a.replace(/(^\s+)|(\s+$)/g,"").replace(/\s+/g," ")}function ba(a,b){b=x(arguments);b[0]=this;a.apply(this,b);return this}
...
function I(a,b,c){var f,d,e,i,g;if(i=/([a-z]+)\((.*)\)/i.exec(a)){g=i[2].split(",");switch(i[1]){case "hsl":return c(parseFloat(g[0]),parseFloat(g[1])/100,parseFloat(g[2])/100);case "rgb":return b(L(g[0]),L(g[1]),L(g[2]))}}if(c=G[a])return b(c.r,c.g,c.b);if(a==o)return b(0,0,0);if(a.charAt(0)=="#"){if(a.length==4){f=a.charAt(1);f+=f;d=a.charAt(2);d+=d;e=a.charAt(3);e+=e}else if(a.length==7){f=a.substring(1,3);d=a.substring(3,5);e=a.substring(5,7)}f=parseInt(f,16);d=parseInt(d,16);e=parseInt(e,16)}return b(f,
d,e)}function va(a,b,c){var f=Math.min(a/=255,b/=255,c/=255),d=Math.max(a,b,c),e=d-f,i=(d+f)/2;if(e){f=i<0.5?e/(d+f):e/(2-d-f);a=a==d?(b-c)/e+(b<c?6:0):b==d?(c-a)/e+2:(a-b)/e+4;a*=60}else f=a=0;return M(a,f,i)}function L(a){var b=parseFloat(a);return a.charAt(a.length-1)=="%"?Math.round(b*2.55):b}
...
function M(a,b,c){return{h:a,s:b,l:c,toString:wa}}function wa(){return"hsl("+this.h+","+this.s*100+"%,"+this.l*100+"%)"}function ta(a,b,c){function f(i){if(i>360)i-=360;else if(i<0)i+=360;if(i<60)return d+(e-d)*i/60;if(i<180)return e;if(i<240)return d+(e-d)*(240-i)/60;return d}var d,e;a%=360;if(a<0)a+=360;b=b<0?0:b>1?1:b;c=c<0?0:c>1?1:c;e=c<=0.5?c*(1+b):c+b-c*b;d=2*c-e;return J(Math.round(f(a+120)*255),Math.round(f(a)*255),Math.round(f(a-120)*255))}var P=O([[document]]);P[0].parentNode=document.documentElement;
...
function O(a){function b(d){for(var e=[],i,g,h,j,l=0,p=a.length;l<p;l++){h=a[l];e.push(i=[]);i.parentNode=h.parentNode;i.parentData=h.parentData;for(var q=0,k=h.length;q<k;q++)if(j=h[q]){i.push(g=d(j));if(g&&"__data__"in j)g.__data__=j.__data__}else i.push(o)}return O(e)}function c(d){for(var e=[],i,g,h,j=0,l=a.length;j<l;j++){g=a[j];for(var p=0,q=g.length;p<q;p++)if(h=g[p]){e.push(i=d(h));i.parentNode=h;i.parentData=h.__data__}}return O(e)}function f(d){for(var e=0,i=a.length;e<i;e++)for(var g=a[e],
h=0,j=g.length;h<j;h++){var l=g[h];if(l)return d.call(l,l.__data__,h)}return o}a.select=function(d){return b(function(e){return e.querySelector(d)})};a.selectAll=function(d){return c(function(e){return x(e.querySelectorAll(d))})};a.filter=function(d){for(var e=[],i,g,h,j=0,l=a.length;j<l;j++){g=a[j];e.push(i=[]);i.parentNode=g.parentNode;i.parentData=g.parentData;for(var p=0,q=g.length;p<q;p++)if((h=g[p])&&d.call(h,h.__data__,p))i.push(h)}return O(e)};a.data=function(d,e){function i(k,n){function r(Ga){return k.parentNode.appendChild(Ga)}
...
0,i=a.length;e<i;e++)for(var g=a[e],h=0,j=g.length;h<j;h++){var l=g[h];l&&d.call(l,l.__data__,h)}return a};a.node=function(){return f(function(){return this})};a.attr=function(d,e){function i(){this.removeAttribute(d)}function g(){this.removeAttributeNS(d.space,d.local)}function h(){this.setAttribute(d,e)}function j(){this.setAttributeNS(d.space,d.local,e)}function l(){var q=e.apply(this,arguments);q==o?this.removeAttribute(d):this.setAttribute(d,q)}function p(){var q=e.apply(this,arguments);q==o?
...
o}function h(){(e.apply(this,arguments)?i:g).call(this)}var j=RegExp("(^|\\s+)"+d3.requote(d)+"(\\s+|$)","g");if(arguments.length<2)return f(function(){j.lastIndex=0;return j.test(this.className)});return a.each(typeof e=="function"?h:e?i:g)};a.style=function(d,e,i){function g(){this.style.removeProperty(d)}function h(){this.style.setProperty(d,e,i)}function j(){var l=e.apply(this,arguments);l==o?this.style.removeProperty(d):this.style.setProperty(d,l,i)}if(arguments.length<3)i=o;if(arguments.length<
...
d3.scale.linear=function(){function a(j){return h((j-c)*i)}function b(j){var l=Math.min(c,f),p=Math.max(c,f),q=p-l,k=Math.pow(10,Math.floor(Math.log(q/j)/Math.LN10));j=j/(q/k);if(j<=0.15)k*=10;else if(j<=0.35)k*=5;else if(j<=0.75)k*=2;return{start:Math.ceil(l/k)*k,stop:Math.floor(p/k)*k+k*0.5,q:k}}var c=0,f=1,d=0,e=1,i=1/(f-c),g=(f-c)/(e-d),h=d3.interpolate(d,e);a.invert=function(j){return(j-d)*g+c};a.domain=function(j){if(!arguments.length)return[c,f];c=j[0];f=j[1];i=1/(f-c);g=(f-c)/(e-d);return a};
a.range=function(j){if(!arguments.length)return[d,e];d=j[0];e=j[1];g=(f-c)/(e-d);h=d3.interpolate(d,e);return a};a.ticks=function(j){j=b(j);return d3.range(j.start,j.stop,j.q)};a.tickFormat=function(j){return d3.format(",."+Math.max(0,-Math.floor(Math.log(b(j).q)/Math.LN10+0.01))+"f")};return a};
...
2)i=0;var g=e[0],h=e[1],j=(h-g)/(b.length+i);f=d3.range(g+j*i,h,j);d=j*(1-i);return a};a.rangeBand=function(){return d};return a};d3.scale.category10=function(){return d3.scale.ordinal().range(Ea)};d3.scale.category20=function(){return d3.scale.ordinal().range(Fa)};d3.scale.category20b=function(){return d3.scale.ordinal().range(Ha)};d3.scale.category20c=function(){return d3.scale.ordinal().range(Ia)};
