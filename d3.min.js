(function(){var o=null;d3={version:"0.16.2"};if(!Date.now)Date.now=function(){return+new Date};if(!Object.create)Object.create=function(a){function d(){}d.prototype=a;return new d};function w(a){return Array.prototype.slice.call(a)}function B(a,d){d=w(arguments);d[0]=this;a.apply(this,d);return this}d3.range=function(a,d,e){if(arguments.length==1){d=a;a=0}if(e==o)e=1;if((d-a)/e==Infinity)throw Error("infinite range");var f=[],c=-1,b;if(e<0)for(;(b=a+e*++c)>d;)f.push(b);else for(;(b=a+e*++c)<d;)f.push(b);return f};
...
U[0].parentNode=document.documentElement;d3.select=function(a){return typeof a=="string"?U.select(a):T([[a]])};d3.selectAll=function(a){return typeof a=="string"?U.selectAll(a):T([w(a)])};
function T(a){function d(c){for(var b=[],h,g,i,j,n=0,p=a.length;n<p;n++){i=a[n];b.push(h=[]);h.parentNode=i.parentNode;h.parentData=i.parentData;for(var k=0,l=i.length;k<l;k++)if(j=i[k]){h.push(g=c(j));if(g&&"__data__"in j)g.__data__=j.__data__}else h.push(o)}return T(b)}function e(c){for(var b=[],h,g,i,j=0,n=a.length;j<n;j++){g=a[j];for(var p=0,k=g.length;p<k;p++)if(i=g[p]){b.push(h=c(i));h.parentNode=i;h.parentData=i.__data__}}return T(b)}function f(c){for(var b=0,h=a.length;b<h;b++)for(var g=a[b],
...
a.range=function(j){if(!arguments.length)return[c,b];c=j[0];b=j[1];g=(f-e)/(b-c);i=d3.interpolate(c,b);return a};a.ticks=function(j){j=d(j);return d3.range(j.start,j.stop,j.k)};a.tickFormat=function(j){return d3.format(",."+Math.max(0,-Math.floor(Math.log(d(j).k)/Math.LN10+0.01))+"f")};return a};
...
d3.svg={};
d3.svg.arc=function(){function a(b){return b.endAngle}function d(b){return b.startAngle}function e(b){return b.outerRadius}function f(b){return b.innerRadius}function c(b){var h=f(b),g=e(b),i=d(b)-Math.PI/2,j=a(b)-Math.PI/2;b=j-i;var n=Math.cos(i);i=Math.sin(i);var p=Math.cos(j);j=Math.sin(j);return"M"+g*n+","+g*i+"A"+g+","+g+" 0 "+(b<Math.PI?"0":"1")+",1 "+g*p+","+g*j+"L"+h*p+","+h*j+"A"+h+","+h+" 0 "+(b<Math.PI?"0":"1")+",0 "+h*n+","+h*i+"Z"}c.innerRadius=function(b){f=typeof b=="function"?b:function(){return b};
